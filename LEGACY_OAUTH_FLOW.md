# Wix OAuth Legacy Flow (Custom Authentication)

> **Note**: This document describes the legacy OAuth flow for Wix integrations. For new integrations, we recommend using the [simplified OAuth flow](./README.md) which is easier to implement and requires no URL configuration in the Wix Dev Center.

## Overview

The legacy Wix OAuth flow uses **Custom Authentication** which differs from standard OAuth providers in several ways:
...
5. **Required URL configuration**: You must configure App URL and Redirect URL in the Wix Dev Center
...
## Comparison with Simplified Flow

| Aspect | Simplified Flow | Legacy Flow |
|--------|-----------------|-------------|
| Installer URL | `https://www.wix.com/app-installer` | `https://www.wix.com/installer/install` |
| Redirect steps | 1 | 2 |
| Dev Center configuration | Not required | Required (App URL + Redirect URL) |
| Token parameter | Not used | Required (must pass back to installer) |
| State management | Via `postInstallationUrl` query params | Manual session management |
...
| Aspect | Standard OAuth | Wix Legacy OAuth |
|--------|---------------|------------------|
